...
def parse_launch_date(val):
    """Convert various date formats to DD-MMM-YYYY.  Returns None for exclusions."""
    ...
    for fmt in ("%d/%m/%Y", "%d-%m-%Y", "%Y-%m-%d", "%d %b %Y", "%d-%b-%Y", "%d/%m/%y"):
        try:
            ...
            pass
    # if it's purely text (no digits or mixed) → exclude
...
def run_automation(config):
    ...
    ecom_tier_ref = config["ecom_tier_ref"]
    ...
    ecom_file.seek(0)
    ecom_df = read_file(ecom_file, sheet_name=ecom_tab)
    if ecom_df is None:
        return None, [f"❌ Could not read Ecom Tracker tab '{ecom_tab}'."]
    logs.append(f"✅ Ecom Tracker tab '{ecom_tab}' loaded: {len(ecom_df)} rows")

    # find STYLE# column in ecom
    style_col = next((c for c in ecom_df.columns if "style#" in c.lower() or "style_#" in c.lower() or c.lower() == "style"), None)
    if style_col is None:
        style_col = next((c for c in ecom_df.columns if "alu" in c.lower()), None)
    if style_col is None:
    ...
    # ── 4. Read Ecom Tier Price Reference ────────────────────────────────────
    tier_sheets = get_excel_sheets(ecom_tier_ref)
    tier_tab = next((s for s in tier_sheets if "reference" in s.lower()), None)
    if tier_tab is None:
        tier_tab = tier_sheets[0] if tier_sheets else None
    ecom_tier_ref.seek(0)
    tier_df = read_file(ecom_tier_ref, sheet_name=tier_tab) if tier_tab else read_file(ecom_tier_ref)
    if tier_df is None:
        return None, ["❌ Could not read Ecom Tracker Price Tier Reference."]

    # find the price tier column that matches the given range (e.g., "AX - BA")
    ref_col = next((c for c in tier_df.columns if "reference" in c.lower()), None)
    if ref_col is None:
        ref_col = tier_df.columns[0]
    # find row where reference matches price_tier
    tier_row = tier_df[tier_df[ref_col].str.upper().str.strip() == price_tier]
    if tier_row.empty:
        logs.append(f"⚠️  Price tier '{price_tier}' not found in reference file — using all ecom rows.")
        rrp_col_name  = next((c for c in ecom_df.columns if "rrp" in c.lower()), None)
        srp_col_name  = next((c for c in ecom_df.columns if "srp" in c.lower()), None)
        disc_col_name = next((c for c in ecom_df.columns if "disc" in c.lower() or "discount" in c.lower()), None)
        excl_col_name = next((c for c in ecom_df.columns if "exclusion" in c.lower()), None)
    else:
        # from tier reference, get column names that map to ecom columns
        # assume tier_df has columns like: Reference | RRP Col | SRP Col | DISC Col | Exclusion Col
        cols = [c for c in tier_df.columns if c != ref_col]
        rrp_col_name  = str(tier_row[cols[0]].values[0]).strip() if len(cols) > 0 else None
        srp_col_name  = str(tier_row[cols[1]].values[0]).strip() if len(cols) > 1 else None
        disc_col_name = str(tier_row[cols[2]].values[0]).strip() if len(cols) > 2 else None
        excl_col_name = str(tier_row[cols[3]].values[0]).strip() if len(cols) > 3 else None
        logs.append(f"✅ Price tier '{price_tier}': RRP='{rrp_col_name}', SRP='{srp_col_name}', DISC='{disc_col_name}', Excl='{excl_col_name}'")
    ...
    def is_eligible_base(row):
        ...
        # Launch date exclusion
        ld = str(row.get("Launch date", "")).strip().lower()
        if ld == "" or "future launch" in ld:
            return False

        # Ecom Status
        es = str(row.get("Ecom Status", "")).strip().upper()
        if es != "YES":
            return False

        # RRP
        try:
            rrp_val = float(str(row.get("RRP", "0")).replace(",", "").strip() or 0)
        except:
            rrp_val = 0
        if rrp_val <= 16 or rrp_val == 0:
            return False

        # SRP
        try:
            srp_val = float(str(row.get("SRP", "0")).replace(",", "").strip() or 0)
        except:
            srp_val = 0
        if srp_val <= 16 and srp_val != 0:
            return False
    ...
    for v in voucher_rows:
        ...
        for idx, row in sc_df.iterrows():
            ...
            # Exclusion keyword filter (from Ecom tracker Exclusion column)
            excl_val = str(row.get("Exclusion", "")).strip().lower()
            if keywords:
                # must match at least one keyword exactly
                if not any(excl_val == kw for kw in keywords):
                    continue

            # AM Exclusion
            am_val = str(row.get("AM exclude", "")).strip()
            alu    = str(row.get("ALU", "")).strip()
            if am_val and am_exclusion_applies(alu, am_val, vpct):
                continue

            sc_df.at[idx, vname] = "YES"
            count += 1
...
with col_d:
    ecom_tier_ref = st.file_uploader("Ecom Tracker Price Tier Reference", type=["csv","xlsx"])
with col_e:
...
if run_btn:
    ...
    if ecom_tier_ref is None: errors.append("Ecom Tracker Price Tier Reference is required.")
